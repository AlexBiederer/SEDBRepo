...
-- Q5: Überhangmandate
numUeberhangMandateProParteiProBL(partei, parteiName, bundesland, numMandate) as
(
	select s.partei, p17.name, s.bundesland, (ms.numSitze - s.numSitze)
    from minSitzeProParteiProBL ms, sitzeProParteiProBL s, partei17 p17
    where s.partei = ms.partei and s.partei = p17.id
